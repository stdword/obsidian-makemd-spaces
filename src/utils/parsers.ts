...
export const parseProperty = (field: string, value: any, type?: string): string => {
    ...
    switch (YAMLtype) {
        ...
        case "date":
            ...
            break;
        case "duration":
            return serializeMultiDisplayString(Object.keys(value.values).reduce((p, c) => [...p, ...(value.values[c] > 0 ? [value.values[c] + " " + c] : [])], []));
            break;
        case "option-multi":
        case "link-multi":
            ...
            return serializeMultiString(
                value.map((v: any) => {
                    ...
                    if (Array.isArray(value) && v.length == 1 && Array.isArray(v[0]) && v[0].length == 1 && typeof v[0][0] === "string") {
                        return v[0][0];
                    }
...
export const parseLinkString = (string: string) => {
    ...
    const stringValue = match?.length > 1 ? match[1].substring(0, indexOfCharElseEOS("|", match[1])) : string;
