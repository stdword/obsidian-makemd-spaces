...
export const getGradientPalettes = (superstate: Superstate): ColorPaletteAsset[] => {
    ...
    return assetManager.getColorPalettes().filter((palette: ColorPaletteAsset) => palette.gradients && palette.gradients.length > 0) || [];
};
...
export const getAllGradients = (superstate: Superstate): ColorPaletteColor[] => {
    ...
    const gradientsFromGradientsProp = palettes.flatMap(
        (p) =>
            p.gradients?.map((g) => ({
                name: g.name,
    ...
    const gradientsFromColors = palettes.flatMap(
        (p) =>
            p.colors
                ?.filter((c) => c.value && (c.value.includes("linear-gradient") || c.value.includes("radial-gradient") || c.value.includes("conic-gradient")))
                .map((c) => ({
                    name: c.name,
                    value: c.value,
