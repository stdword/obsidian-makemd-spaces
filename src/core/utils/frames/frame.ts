...
export const executableChanged = (a: FrameExecutable, b: FrameExecutable) => {
    return !_.isEqual(deepOmit(a, ["execPropsOptions", "execProps", "execStyles", "execActions", "parent"]), deepOmit(b, ["execPropsOptions", "execProps", "execStyles", "execActions", "parent"]));
};

export const stateChangedForProps = (propSetters: string[], props: FrameTreeProp, newState: FrameTreeProp, schemaID: string) => {
    return propSetters.filter((f) => newState[schemaID]?.props[f] && !_.isEqual(newState[schemaID].props[f], props?.[f]));
};
...
const saveFrameRoot = async (superstate: Superstate, tableData: SpaceTable, space: SpaceInfo, frameRoot: FrameRoot) => {
    ...
    const newTable = {
        ...tableData,
        cols: tableData.cols ?? [],
        rows: [...treeNodes].map((f) => nodeToFrame(relinkProps("$root", tableData.schema.id, f, tableData.schema.id))) as MFrame[],
    };
...
export const replaceFrameWithFrameRoot = async (superstate: Superstate, space: SpaceInfo, schema: string, root: FrameRoot) => {
    return superstate.spaceManager.readFrame(space.path, schema).then((tagDB) => saveFrameRoot(superstate, tagDB, space, root));
};
