...
const DefaultSpaceCols: SpaceProperty[] = [
    {
        name: "space",
        type: "space",
        schemaId: "main",
        value: JSON.stringify({ alias: i18n.defaults.spaceContext }),
    },
    {
        name: "note",
        type: "link",
        schemaId: "main",
        value: JSON.stringify({ alias: i18n.defaults.spaceNote }),
    },
];
...
const processTable = async (manager: SpaceManager, space: SpaceInfo, table: string, processor: (mdb: SpaceTable, space: SpaceInfo) => Promise<SpaceTable>): Promise<void> => {
    const contextDB = await manager.readTable(space.path, table);
    if (contextDB) {
        await processor(contextDB, space);
    }
};

const processContext = async (manager: SpaceManager, space: SpaceInfo, processor: (mdb: SpaceTable, space: SpaceInfo) => Promise<SpaceTable>): Promise<void> => {
...
const saveContext = async (manager: SpaceManager, spaceInfo: SpaceInfo, newTable: SpaceTable, forceCreate?: boolean, calculate = true): Promise<void> => {
...
export const insertPropertyMultiValue = (folder: SpaceTable, lookupField: string, lookupValue: string, field: string, value: string) => {
    return {
        ...folder,
...
export const deletePropertyMultiValue = (folder: SpaceTable, lookupField: string, lookupValue: string, field: string, value: string) => {
    return {
        ...folder,
        rows: folder.rows.map((f) =>
            f[lookupField] == lookupValue
...
const updateValue = (folder: SpaceTable, lookupField: string, lookupValue: string, field: string, value: string) => {
    return {
        ...folder,
        rows: folder.rows.map((f) =>
            f[lookupField] == lookupValue
...
export const updateTableValue = async (manager: SpaceManager, space: SpaceInfo, schema: string, index: number, field: string, value: string, rank?: number) => {
...
export const updateContextValue = async (
    ...
    field: string,
    value: string,
    _updateFunction?: (folder: SpaceTable, lookupField: string, lookupValue: string, field: string, value: string) => SpaceTable,
    rank?: number,
    force?: boolean,
    calculate?: boolean,
): Promise<void> => {
    return manager.contextForSpace(space.path).then((f) => {
        ...
        if (rank) newMDB = reorderRowsForPath(newMDB, [path], rank);
        return saveContext(manager, space, newMDB, force, calculate);
    });
};

export const columnsForContext = async (manager: SpaceManager, space: SpaceInfo): Promise<SpaceProperty[]> => {
    return manager.contextForSpace(space.path).then((tagDB) => tagDB?.cols ?? []);
};
...
export const updateTableRow = async (manager: SpaceManager, space: SpaceInfo, table: string, index: number, row: DBRow): Promise<void> => {
...
export const updateValueInContext = async (manager: SpaceManager, row: string, field: string, value: string, space: SpaceInfo): Promise<void> => {
...
export const renameTagInContexts = async (manager: SpaceManager, oldTag: string, newTag: string, spaces: SpaceInfo[]): Promise<void[]> => {
    const changeTagInContextMDB = (mdb: SpaceTable) => {
...
export const removeTagInContexts = async (manager: SpaceManager, tag: string, spaces: SpaceInfo[]): Promise<void[]> => {
...
export const addRowInTable = async (manager: SpaceManager, row: DBRow, context: SpaceInfo, table: string, index?: number): Promise<void> => {
...
export const addPathInContexts = async (manager: SpaceManager, path: string, contexts: SpaceInfo[], index?: number): Promise<void[]> => {
...
export const renameLinkInContexts = async (manager: SpaceManager, oldPath: string, newPath: string, spaces: SpaceInfo[]): Promise<void[]> => {
    const promises = spaces.map((space) => {
        return processContext(manager, space, async (mdb, space) => {
...
export const removeLinkInContexts = async (manager: SpaceManager, path: string, spaces: SpaceInfo[]): Promise<void[]> => {
    const promises = spaces.map((space) => {
        return processContext(manager, space, async (mdb, space) => {
            const linkCols = linkColumns(mdb.cols);
            const newDB = {
...
export const renamePathInContexts = async (manager: SpaceManager, oldPath: string, newPath: string, spaces: SpaceInfo[]): Promise<void[]> => {
    const promises = spaces.map((space) => {
        return processContext(manager, space, async (mdb, space) => {
...
export const removePathInContexts = async (manager: SpaceManager, path: string, spaces: SpaceInfo[]): Promise<void[]> => {
    const promises = spaces.map((space) => {
        return processContext(manager, space, async (mdb, space) => {
...
export const reorderPathsInContext = async (manager: SpaceManager, paths: string[], index: number, space: SpaceInfo): Promise<void> => {
...
export const removePathsInContext = async (manager: SpaceManager, paths: string[], space: SpaceInfo): Promise<void> => {
    return processContext(manager, space, async (mdb, context) => {
