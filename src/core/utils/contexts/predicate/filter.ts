...
export type FilterFunctionType = Record<string, { type: string[]; fn: FilterFunction; valueType: string }>;
type FilterFunction = (v: any, f: any) => boolean;

export const startsWith: FilterFunction = (value: string, filterValue: string): boolean => {
    return (value ?? "").startsWith(filterValue);
};

export const endsWith: FilterFunction = (value: string, filterValue: string): boolean => {
    return (value ?? "").endsWith(filterValue);
};

export const lengthEquals: FilterFunction = (value: string, filterValue: string): boolean => {
    return value.length == parseInt(filterValue);
};

export const listEquals: FilterFunction = (value: string, filterValue: string): boolean => {
...
export const stringEqual: FilterFunction = (value: string, filterValue: string): boolean => {
...
export const stringCompare: FilterFunction = (value: string, filterValue: string): boolean => {
    return (value ?? "").toLowerCase().includes((filterValue ?? "").toLowerCase());
};

export const greaterThan: FilterFunction = (value: string, filterValue: string): boolean => {
    return parseFloat(value) > parseFloat(filterValue);
};

export const lessThan: FilterFunction = (value: string, filterValue: string): boolean => {
    return parseInt(value) < parseInt(filterValue);
};
export const dateAfter: FilterFunction = (value: string, filterValue: string): boolean => {
...
export const dateBefore: FilterFunction = (value: string, filterValue: string): boolean => {
    const dateValue = isNaN(Date.parse(value)) ? new Date(parseInt(value)) : new Date(value);
    const dateFilterValue = isNaN(Date.parse(filterValue)) ? new Date(parseInt(filterValue)) : new Date(filterValue);
    return dateValue.valueOf() < dateFilterValue.valueOf();
};

export const listIncludes: FilterFunction = (value: string, filterValue: string): boolean => {
    const valueList = value ? parseMultiString(value) : [];
    const strings = filterValue ? parseMultiString(filterValue) : [];
...
export const filterReturnForCol = (col: SpaceTableColumn, filter: Filter, row: DBRow, properties: Record<string, any>) => {
    if (!col) return true;
