...
export const wrapObjectString = (s: string) =>
    `{ ${Object.entries(safelyParseJSON(s))
...
export const removeLeadingSlash = (path: string) => (path.charAt(0) == "/" ? path.substring(1) : path);
export const pathToParentPath = (path: string) => removeLeadingSlash(path.substring(0, path.lastIndexOf("/"))) || path;
