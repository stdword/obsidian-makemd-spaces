...
import { Gradient, parseGradient, stringifyGradient } from "core/utils/color/gradient";
...
const ColorSwatch: React.FC<{
    ...
    return (
        <div className={`mk-color-swatch ${sizeClasses[size]} ${className} ${isGradient ? "mk-gradient-swatch" : ""}`}>
                    ...
                    <div key={palette.id} className="mk-palette-section">
                        ...
                        <div className="mk-palette-colors">
                            {palette.colors.map((color: any, index: number) => {
                                const isGradient = color.value && (color.value.includes("linear-gradient") || color.value.includes("radial-gradient") || color.value.includes("conic-gradient"));
                                return <ColorSwatch key={`${palette.id}-${index}`} color={color.value} name={color.name} size="medium" isGradient={isGradient} onClick={() => onColorSelect(color.value)} />;
                            })}
                        </div>
                    </div>
                ))
...
export const ColorPicker = (props: { superstate: Superstate; color: string; hide?: () => void; saveValue: (color: string) => void; stayOpen?: boolean; allowGradient?: boolean; hidePaletteSelector?: boolean }) => {
    ...
    const updateColor = (color: string) => {
        ...
        const isGradientString = color && (color.includes("linear-gradient(") || color.includes("radial-gradient(") || color.includes("conic-gradient(") || color.includes("repeating-linear-gradient(") || color.includes("repeating-radial-gradient("));
            ...
            try {
                ...
                if (gradientObject) {
                    const values = gradientObject.values.map((g) => {
                        const existingStop = gradient?.values.find((f) => f.position == g.position && f.color == g.color);
                ...
                <div className="mk-gradient-editor">
                    ...
                    <div
                        ...
                        onClick={(e) => {
                            ...
                            const nearExistingStop = gradient.values.some((stop) => Math.abs(stop.position - position) <= thresholdDistance);
...
export const showColorPickerMenu = (superstate: Superstate, rect: Rect, win: Window, value: string, setValue: (color: string) => void, stayOpen?: boolean, isSubmenu?: boolean, hidePaletteSelector?: boolean) => {
    return superstate.ui.openCustomMenu(rect, <ColorPicker superstate={superstate} color={value} saveValue={setValue} stayOpen={stayOpen} hidePaletteSelector={hidePaletteSelector}></ColorPicker>, {}, win, "bottom");
};
