...
export const showLinkMenu = (offset: Rect, win: Window, superstate: Superstate, saveLink: (link: string | string[]) => void, options?: Partial<SelectMenuProps>) => {
    ...
    const sections: SelectSection[] = Array.from(new Set(suggestions.map((f) => f.section))).map((f) => {
        return {
            name: f,
            value: f,
        };
