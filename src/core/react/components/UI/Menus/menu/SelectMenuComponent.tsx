...
type SelectMenuComponentProps = {
    ...
    suggestionsTransform?: (query: string, suggestions: SelectOption[]) => SelectOption[];
    delimiters?: string[];
...
const pressDelimiter = (props: SelectMenuComponentProps, query: string, index: number, options: SelectOption[], addTag: (tag: SelectOption, modifiers: PointerModifiers) => void, modifiers: PointerModifiers) => {
...
const SelectMenuComponent = React.forwardRef((_props: SelectMenuComponentProps, ref: any) => {
    ...
    const sections: SelectSection[] = useMemo(() => {
        ...
        return [
            ...
            ...(props.sections ?? uniq(props.suggestions.map((f) => f.section)).filter((f) => f)),
        ];
    ...
    const onInput = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (_query.length === query.length + 1 && props.delimiters.indexOf(query.slice(-1)) > -1) {
                    ...
                    {props.tags.length > 0 && (
                        <div className={classNames.selectedTagWrapper}>
                            {props.tags.map((tag, i) => (
                                <SelectMenuPillComponent key={i} tag={tag} classNames={classNames} onDelete={(e) => onDeleteTag(i, e)} />
                            ))}
                        </div>
                    )}
