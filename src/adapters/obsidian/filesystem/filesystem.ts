...
export class ObsidianFileSystem implements FileSystemAdapter {
    ...
    public parentPathForPath(path: string) {
        return removeTrailingSlashFromFolder(getParentPathFromString(path));
    }
    public resolvePath(path: string, source: string) {
        ...
        if (uri.refStr?.length > 0) {
            if (uri.refType == "block" || uri.refType == "heading") {
                ...
                if (resolvedPath) return resolvedPath + "#" + uri.refStr;
            }
            return path;
        }
    ...
    public async copyFile(path: string, folder: string, newName?: string) {
        ...
        if (file.isFolder) {
            if (await this.fileExists(newPath)) {
                ...
                newName = uniqueNameFromString(
                    file.name,
                    folders.map((f) => f.split("/").pop()),
                );
            ...
            try {
                if (await this.fileExists(newPath)) {
                    ...
                    const newName = uniqueNameFromString(
                        file.name,
    ...
    public async readTextFromFile(path: string) {
        const file = this.plugin.app.vault.getAbstractFileByPath(path) as TFile;
        if (file) return this.plugin.app.vault.read(file);
        if (await this.fileExists(path)) return this.plugin.app.vault.adapter.read(path);
        return null;
    }
    ...
    public async childrenForFolder(path: string, type?: string) {
        if (type == "folder") {
            const g = await this.plugin.app.vault.adapter.list(path);
            return g.folders;
        } else if (type == "file") {
            const g = await this.plugin.app.vault.adapter.list(path);
            return g.files;
