...
export class ObsidianMarkdownFiletypeAdapter implements FileTypeAdapter<CleanCachedMetadata, CachedMetadataContentTypes> {
    ...
    public async parseCache(file: AFile, refresh?: boolean) {
        ...
        if (fCache && fCache.tags) rt.push(...(fCache.tags?.map((f) => f.tag) ?? []));
        if (fCache && fCache.frontmatter?.tags)
            rt.push(...(typeof fCache.frontmatter?.tags === "string" ? parseMultiDisplayString(fCache.frontmatter.tags.replace(/ /g, "")) : Array.isArray(fCache.frontmatter?.tags) ? (fCache.frontmatter?.tags ?? []) : []).filter((f) => typeof f === "string").map((f) => ensureTag(f)));
        if (fCache && fCache.frontmatter?.tag)
            rt.push(...(typeof fCache.frontmatter?.tag === "string" ? parseMultiDisplayString(fCache.frontmatter.tag.replace(/ /g, "")) : Array.isArray(fCache.frontmatter?.tag) ? (fCache.frontmatter?.tag ?? []) : []).filter((f) => typeof f === "string").map((f) => ensureTag(f)));
    ...
    public async readContent(file: AFile, fragmentType: keyof CachedMetadataContentTypes, fragmentId: any) {
        ...
        if (fragmentType == "frontmatter" || fragmentType == "property") {
            ...
            const rows = fmKeys.reduce((p, c) => ({ ...p, [c]: parseProperty(c, fm[c]) }), {});
            return rows;
        }
                ...
                if (this.app.fileManager.processFrontMatter) {
                    await this.app.fileManager.processFrontMatter(afile, (frontmatter: any) => {
                        ...
                        Object.keys(frontmatter)
                            .filter((f) => !newKeys.includes(f))
                            .forEach((f) => delete frontmatter[f]);
                    });
                }
            }
        }
