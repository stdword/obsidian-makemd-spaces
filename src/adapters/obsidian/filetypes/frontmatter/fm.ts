...
export const mergeTableData = (mdb: SpaceTable, yamlmdb: DBTable, types: Record<string, string>): SpaceTable => {
    return {
        ...mdb,
        cols: [
            ...mdb.cols,
...
export const renameFrontmatterKey = (plugin: MakeMDPlugin, path: string, key: string, name: string) => {
    plugin.superstate.spaceManager.renameProperty(path, key, name);
};

export const changeFrontmatterType = (plugin: MakeMDPlugin, path: string, key: string, type: string) => {
    saveProperties(plugin.superstate, path, {
        [key]: defaultValueForType(type),
    });
};

export const deleteFrontmatterValue = (plugin: MakeMDPlugin, path: string, key: string) => {
    plugin.superstate.spaceManager.deleteProperty(path, key);
};

export const saveFrontmatterValue = (plugin: MakeMDPlugin, path: string, key: string, value: string, type: string) => {
    saveProperties(plugin.superstate, path, {
