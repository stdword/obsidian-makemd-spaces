...
export const getMDB = async (plugin: MDBFileTypeAdapter, path: string): Promise<MDB> => {
    ...
    try {
        dbTable = schemas
            .filter((f) => f.type == "db")
            .map((f) => ({ [f.id]: dbResultsToDBTables(db.exec(`SELECT * FROM "${f.id}"`))[0] }))
            .reduce((p, c) => ({ ...p, ...c }), {});
    } catch (e) {
        db.close();
        return null;
...
export const getMDBTable = async (adapter: MDBFileTypeAdapter, dbPath: string, table: string): Promise<SpaceTable> => {
    ...
    const fields = ((fieldsTables[0]?.rows as SpaceProperty[]) ?? []).filter((f) => f.name.length > 0);
    let dbTable;
    try {
...
export const getMDBTables = async (plugin: MDBFileTypeAdapter, dbPath: string) => {
    ...
    if (schemas.length == 0) {
        ...
        db.exec(`CREATE TABLE IF NOT EXISTS m_schema ('id' char, 'name' char, 'type' char, 'def' char, 'predicate' char, 'primary' char)`);
