...
export const savePropertyToDBTables = (newColumn: SpaceProperty, fields: SpaceProperty[], oldColumn?: SpaceProperty): DBTables => {
    ...
    const oldFieldIndex = oldColumn ? fields.findIndex((f) => f.name == oldColumn.name) : -1;
    const newFields: SpaceProperty[] = oldFieldIndex == -1 ? [...fields, column] : fields.map((f, i) => (i == oldFieldIndex ? column : f));
    return {
        m_fields: {
